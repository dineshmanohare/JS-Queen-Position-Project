...
function getStart() {
    ...
    if (direction != "") {
        getDirection(direction, steps);
    }


    function getDirection(direction, steps) {
        switch (direction) {
            case 'E': east(steps);
                break;
            case 'W': west(steps);
    ...
    function east(steps) {
        ...
        if (column_result <= 7) {
            ...
            alert("queen position is " + position)
        }
        else {
            alert("queen is going out side of board");
        }
    ...
    queen.loopContinueResult = window.prompt("do you want to continue yes/no ");

    if (queen.loopContinueResult == 'yes') {
        getStart();
    }
}
